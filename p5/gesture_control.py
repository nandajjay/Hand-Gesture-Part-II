    ...
    def get_frame_with_detections(self):
        with self.lock:
            if self.frame is not None:
                nparr = np.frombuffer(self.frame, np.uint8)
                frame_np = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
                frame_with_detections = self.detect_objects(frame_np)
                ret, buffer = cv2.imencode('.jpg', frame_with_detections)
                return buffer.tobytes(), self.obj_detections
            return None, []
